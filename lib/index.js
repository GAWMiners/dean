...
function Dean(opts) {
  ...
  this.key = opts.key || opts['session-key'] || null
  ...
  if (opts.https) {
    if (!opts.https.key) throw new Error('https.key is required')
    if (!opts.https.cert) throw new Error('https.cert is required')
    debug('using https')
    this.server = https.createServer(opts.https, wrappedSite)
  } else {
    debug('using http')
